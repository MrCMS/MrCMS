...
{
    public class ArticleSectionTreeNavListing :WebpageTreeNavListing<ArticleSection>
    {
        private readonly ISession _session;
        private readonly UrlHelper _urlHelper;

        public ArticleSectionTreeNavListing(ISession session,UrlHelper urlHelper)
        {
            _session = session;
        ...
        public override AdminTree GetTree(int? id)
        {
            ...
            var query = _session.QueryOver<ArticleSection>().Where(x => x.Parent.Id == id);
            if (id.HasValue)
            {
                if (parent != null)
                {
            ...
            query.Cacheable().List().ForEach(doc =>
            {
                ...
                var node = new AdminTreeNode
                {
                    ...
                    HasChildren = _session.QueryOver<ArticleSection>().Where(webpage => webpage.Parent.Id == doc.Id).Cacheable().Any(),
