(function ($) {
    ...
    function setupSelector(div) {

        $(div).on('click', '#media-selector .selected-file', function () {
    ...
    function updateFiles(callback) {
        ...
        $('#library').load(settings.mediaSelectorUrl + ' div#library', { categoryId: categoryId, imagesOnly: imagesId, v: new Date().getTime() }, function () {
            resizeModal($(this));
            $(".set-file").attr('disabled', 'disabled');
            if (callback)
                callback();
