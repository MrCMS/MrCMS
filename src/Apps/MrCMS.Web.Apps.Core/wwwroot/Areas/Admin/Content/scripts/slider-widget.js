...
function initInputs() {
    ...
    sliders.each(function (index) {
        ...
        let caption = inputSelector.find('[id="slider-caption"]');
        let link = inputSelector.find('input[id="slider-link"]');

        let captionIsTextArea = caption[0] instanceof HTMLTextAreaElement;
        ...
        inputSelector.find('input[value="Add"]').on("click", function () {
            if (image.val().length > 0) {
                ...
                let captionHtml='';
                if (captionIsTextArea){
                    let ckEditorInstance=CKEDITOR.instances["slider-caption"];
                    captionHtml = html`${ckEditorInstance.getData()}<input type="hidden" name="${"SlideList[" + index + "].Caption"}" value='${ckEditorInstance.getData()}'/>`;
                }
                else{
                    captionHtml = html`${caption.val()}<input type="hidden" name="${"SlideList[" + index + "].Caption"}" value='${caption.val()}'/>`;
                }
