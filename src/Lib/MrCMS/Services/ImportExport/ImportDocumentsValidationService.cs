...
namespace MrCMS.Services.ImportExport
{
    public class ImportDocumentsValidationService : IImportDocumentsValidationService
    {
        ...
        /// <returns></returns>
        public async Task<(List<DocumentImportDTO> data, Dictionary<string, List<string>> parseErrors)>
        ...
        private async Task<(DocumentImportDTO item, List<string> errors)> GetDocumentImportDataTransferObject(
            ...
            string name)
        {
            ...
            {
                item.DocumentType = worksheet.Cell(rowId, 3).GetValue<string>();
