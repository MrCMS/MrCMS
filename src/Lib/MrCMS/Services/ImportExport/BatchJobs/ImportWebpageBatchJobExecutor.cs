...
{
    public class ImportWebpageBatchJobExecutor : BaseBatchJobExecutor<ImportWebpageBatchJob>
    {
        ...
        public ImportWebpageBatchJobExecutor(ISession session,
        ...
        protected override async Task<BatchJobExecutionResult> OnExecuteAsync(ImportWebpageBatchJob batchJob)
        {
            ...
            using (_eventContext.Disable<ShowAddedNotification>())
            using (_eventContext.Disable<MediaCategoryUpdatedNotification>())
            {
                var documentImportDto = batchJob.WebpageImportDto;
                var webpage =
                ...
                if (isNew)
                    webpage = (Webpage)
                        Activator.CreateInstance(
                            _webpageMetadataService.GetTypeByName(documentImportDto.WebpageType));
