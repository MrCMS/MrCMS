        ...
        public override async IAsyncEnumerable<SEOAnalysisFacet> GetFacets(Webpage webpage, HtmlNode document, string analysisTerm)
        {
            ...
            if (titleText != null && titleText.Contains(analysisTerm, StringComparison.OrdinalIgnoreCase))
            {
                if (titleText.StartsWith(analysisTerm))
                    yield return
                        await GetFacet("Title contains term", SEOAnalysisStatus.Success,
                            string.Format("The title starts with the term '{0}', which is considered to improve rankings", analysisTerm));
                else
                {
                    yield return
                        await GetFacet("Title contains term", SEOAnalysisStatus.CanBeImproved,
                            string.Format(
                ...
                yield return
                    await GetFacet("Title contains term", SEOAnalysisStatus.Error,
