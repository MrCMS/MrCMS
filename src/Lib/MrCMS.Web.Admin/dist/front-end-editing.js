...
})({"b4ANS":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "b205c4de908c33ad";
"use strict";
...
},{}],"cutHe":[function(require,module,exports) {
var _setupFeatherlight = require("./setup-featherlight");
let layoutAreas;
let documentBody;
...
function initEditing() {
    ...
    documentBody = document.getElementsByTagName("body");
    ...
    enableEditingBtn.addEventListener("click", function() {
        if (!getEditingEnabled()) enableEditors();
        else disableEditing();
    });
    documentBody[0].classList.add('mrcms-admin-bar-on');
}
function enableEditors() {
    documentBody[0].classList.add("editing-on");
    setEditingEnabled(true);
    enableEditingBtn.innerText = "Inline Editing: On";
    ...
    editors.forEach((x)=>{
        ...
        if (el.dataset.isHtml) {
            ...
            editor.on('blur', function(e) {
                if (original !== e.editor.getData()) $.ajax({
                    type: "POST",
                    url: "/Admin/InPageAdmin/SaveContent",
                    headers: {
                        AntiForgeryToken: documentBody[0].dataset.antiforgeryToken
                    },
                    data: {
                        id: el.dataset.id,
                        property: el.dataset.property,
                    ...
                    success: function(msg) {
                        if (msg.success === false) alert(msg.message);
                        else showLiveForm(el);
                    }
                });
                else showLiveForm(el);
            });
    ...
    document.querySelectorAll("div[data-layout-area-id]").forEach((x)=>{
        let html = "";
        if (x.offsetHeight === 0) html = "<div class='edit-indicator-layout corner'><img src='/Areas/Admin/Content/img/layout-2.png' /></div>";
        else html = "<div class='edit-indicator-layout'><img src='/Areas/Admin/Content/img/layout-1.png' /></div>";
        x.insertAdjacentHTML('afterbegin', html);
    });
    ...
    editWidgetIndicators.forEach((x1)=>x1.addEventListener('click', function(x) {
            ...
            document.querySelectorAll('.mrcms-edit-widget')[0].display = 'block';
            //if click outside hide the menu
            $(document).mouseup(function(e) {
                const container = $(".mrcms-edit-widget", document);
                if (container.has(e.target).length === 0) container.remove();
            });
        })
    );
    ...
    $('.edit-indicator-layout', document).click(function() {
        ...
        $('[data-action=post-link]').on('click', function(e) {
            ...
            if (url != null) window.postToUrl(url, {
            });
        });
    });
...
function disableEditing() {
    setEditingEnabled(false);
    documentBody[0].classList.remove("editing-on");
    layoutAreas.forEach((x)=>x.classList.remove('layout-area-enabled')
    );
    enableEditingBtn.innerText = "Inline Editing: Off";
    ...
    document.querySelectorAll(editableSelector).forEach((x)=>{
        if (x.dataset.isHtml === true) showLiveForm(x);
    });
...
function killCkeditors() {
    ...
    if (instances) for(let k in instances){
...
const MrCMSFeatherlightSettings = {
    ...
    iframeWidth: 800,
    afterOpen: function() {
        _setupFeatherlight.setCloseButtonPosition(this.$instance);
    },
    beforeOpen: function() {
...
$(function() {
    initEditing();
    const featherlightSettings = $.extend({
    }, MrCMSFeatherlightSettings, {
...
},{"./setup-featherlight":"gmK6R"}],"gmK6R":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "setCloseButtonPosition", ()=>setCloseButtonPosition
);
parcelHelpers.export(exports, "getRemoteModel", ()=>getRemoteModel
);
parcelHelpers.export(exports, "setupFeatherlight", ()=>setupFeatherlight
);
const MrCMSFeatherlightSettings = {
    type: 'iframe',
    iframeWidth: 800,
...
},{}]},["b4ANS","cutHe"], "cutHe", "parcelRequireaed2")
